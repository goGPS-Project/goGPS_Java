...
//					else if( uid == 0x7) {} // UBX-NAV-PVT
			...
			if (!parsed) {
				...
				if (this.debugModeEnabled) {
					System.out.println("Warning: Message 0x" + Integer.toHexString(uclass) + " 0x"
							+ Integer.toHexString(uid) + " not decoded; skipping " + len + " bytes");
				}
			...
			if (this.debugModeEnabled) {
				System.out.println("Warning: wrong sync char 2 " + usynch2 + " " + Integer.toHexString(usynch2) + " ["
						+ ((char) usynch2) + "]");
			}
		}
		// }else{
		// //no warning, may be NMEA
		// //System.out.println("Warning: wrong sync char 1 "+data+"
		// "+Integer.toHexString(data)+" ["+((char)data)+"]");
		// }
		return null;
	}
